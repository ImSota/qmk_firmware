...
const PROGMEM naginata_keymap ngmap[] = {
  ...
  {.key = B_D|B_M|B_O       , .kana = "nye"     }, // にぇ
};

const PROGMEM naginata_keymap_long ngmapl[] = {
// 編集モード Win/Linux
  // {.key = A_J|A_K|B_Q		, .kana = "dhi"}, // でぃ
...
//{.key = B_J|B_K|B_V		, .kana = SS_TAP(X_ENTER)SS_TAP(NGDN)}, // {改行}{↓}
  {.key = B_J|B_K|B_B		, .kana = SS_TAP(X_END)SS_TAP(X_ENTER)SS_TAP(X_SPACE)}, // {End}{改行}{Space}
  {.key = B_D|B_F|B_Y		, .kana = SS_TAP(X_HOME)}, // {Home}
  {.key = B_D|B_F|B_U		, .kana = SS_LSFT(SS_TAP(X_END))SS_TAP(X_BSPC)}, // +{End}{BS}
  {.key = B_D|B_F|B_I		, .kana = SS_TAP(X_INT4)}, // {vk1Csc079}-----------------------------------------変換をかなにわりあてるとできてない
  {.key = B_D|B_F|B_O		, .kana = SS_TAP(X_DELETE)}, // {Del}
  {.key = B_D|B_F|B_P		, .kana = SS_TAP(X_ESCAPE)SS_TAP(X_ESCAPE)SS_TAP(X_ESCAPE)}, // {Esc 3}
  {.key = B_D|B_F|B_H		, .kana = SS_TAP(X_END)}, // {End}
...
//{.key = B_D|B_F|B_SLSH		, .kana = SS_TAP(NGDN)SS_TAP(NGDN)SS_TAP(NGDN)SS_TAP(NGDN)SS_TAP(NGDN)}, // {↓ 5}
  {.key = B_M|B_COMM|B_Q		, .kana = SS_LCTL("x")SS_TAP(X_BSPC)SS_TAP(X_DELETE)SS_LCTL("v")}, // ^x{BS}{Del}^v
...
//{.key = B_M|B_COMM|B_X		, .macro , // ^x【^v】{改行}{Space}+{↑}^x
  {.key = B_M|B_COMM|B_C		, .kana = SS_TAP(X_HOME)SS_TAP(X_BSPC)SS_TAP(X_DELETE)SS_TAP(X_DELETE)SS_TAP(X_DELETE)SS_TAP(X_END)}, // {Home}{BS}{Del 3}{End}
  {.key = B_M|B_COMM|B_V		, .kana = SS_TAP(X_HOME)SS_TAP(X_BSPC)SS_TAP(X_DELETE)SS_TAP(X_END)}, // {Home}{BS}{Del 1}{End}
//{.key = B_M|B_COMM|B_B		, .macro , // 　　　×　　　×　　　×{改行 2}
  ...
  {.key = B_C|B_V|B_O		, .kana = SS_LCTL("z")}, // ^z
  {.key = B_C|B_V|B_P		, .kana = SS_LCTL("y")}, // ^y
...
// 編集モード 縦横
  ...
  {.key = B_D|B_F|B_J		, .kana = SS_TAP(NGUP)}, // {↑}
  {.key = B_D|B_F|B_K		, .kana = SS_LSFT(SS_TAP(NGUP))}, // +{↑}
  {.key = B_D|B_F|B_L		, .kana = SS_LCTL(SS_LSFT(SS_TAP(NGUP)))}, // +{↑ 単語}
  {.key = B_D|B_F|B_SCLN		, .kana = SS_LCTL(SS_TAP(NGUP))}, // {↑ 単語}
  {.key = B_D|B_F|B_M		, .kana = SS_TAP(NGDN)}, // {↓}
  {.key = B_D|B_F|B_COMM		, .kana = SS_LSFT(SS_TAP(NGDN))}, // +{↓}
  {.key = B_D|B_F|B_DOT		, .kana = SS_LCTL(SS_LSFT(SS_TAP(NGDN)))}, // +{↓ 単語}
  {.key = B_D|B_F|B_SLSH		, .kana = SS_LCTL(SS_TAP(NGDN))}, // {↓ 単語}
...
// 編集モード Mac
  ...
  {.key = B_M|B_COMM|B_Q		, .kana = SS_LCMD("x")SS_TAP(X_BSPC)SS_TAP(X_DELETE)SS_LCMD("v")}, // ^x{BS}{Del}^v
  ...
  {.key = B_C|B_V|B_O		, .kana = SS_LCMD("z")}, // ^z
  {.key = B_C|B_V|B_P		, .kana = SS_LCMD("y")}, // ^y
...
// 編集モードunicode
  {.key = B_J|B_K|B_T		, .win   = "・",		.mac = "nagichuutenn"}, // ・
  {.key = B_J|B_K|B_A		, .win   = "……",		.mac = "nagitentenn",}, // ……{改行}--------------------------------------------------------------------------------------改行いらん
  {.key = B_J|B_K|B_Z		, .win   = "──",		.mac = "nagitatesenni"}, // ―――{改行}-----------------------------------------------------------------------------------改行いらん
  {.key = B_M|B_COMM|B_T		, .win   = "〇",		.mac = "nagimaru"}, // 〇{確定}
  {.key = B_M|B_COMM|B_G		, .win   = "／",		.mac = "naginaname"}, // ／{確定}
};
...
//{.key = B_J|B_K|B_F		, .macro , // 「」{改行}{↑}
    case B_J|B_K|B_F: // 「」{改行}{↑}
      ...
      return true;
      break;
//{.key = B_J|B_K|B_G		, .macro , // {End}{改行}「」{改行}{↑}
    case B_J|B_K|B_G: // {End}{改行}「」{改行}{↑}
      switch (naginata_config.os) {
        case NG_WIN:
        case NG_LINUX:
          // tap_code(KC_ENT);
